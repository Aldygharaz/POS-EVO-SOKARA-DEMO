...
export default function Sidebar({ isMobileOpen, onMobileClose }: SidebarProps) {
  const { currentPage, setCurrentPage, currentUser, hasPermission } = useStore();
  ...
  const renderMenuItem = (item: { page: string; label: string; icon: React.ComponentType<{ className?: string }> }) => {
    ...
    return (
      ...
      >
        <Icon className={`w-[18px] h-[18px] transition-transform duration-200 group-hover:scale-110 ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} />
        <span>{item.label}</span>
        {isActive && <ChevronRight className="w-4 h-4 ml-auto text-emerald-600 dark:text-emerald-400" />}
      </button>
    );
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-200 dark:border-slate-800">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-900 dark:text-slate-100 tracking-tight">Sokara POS</h1>
            <p className="text-[10px] text-emerald-500 font-semibold uppercase tracking-wider">Sokara AI Enterprise</p>
          </div>
        ...
        {/* User Card */}
        <div className="mx-3 mt-3 p-3 rounded-xl bg-slate-50/80 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60">
          <div className="flex items-center gap-2.5">
            <div className="relative w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500/25 to-emerald-500/5 border border-emerald-500/30 flex items-center justify-center">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between">
                ...
                {currentUser?.username === 'demo' && (
                  <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/20 uppercase tracking-wider">
                    Demo
                  </span>
                )}
              </div>
              <p className="text-[10px] font-medium text-slate-500 dark:text-slate-400 capitalize">{currentUser?.role}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-3 border-t border-slate-200/80 dark:border-slate-800/80 flex gap-2">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:text-rose-500 dark:hover:text-rose-400 dark:hover:bg-rose-500/10 rounded-lg transition-colors ${currentUser?.username === 'demo' ? 'flex-[2]' : 'w-full'}`}
          >
